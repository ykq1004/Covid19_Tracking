...
state_shp <- readOGR(dsn="tl_2019_us_state/",  layer="tl_2019_us_state", verbose=FALSE)
   ...
   tabPanel("COVID-19 tracker", 
     ...
     fluidRow(
       radioButtons(inputId = "death_metric", label = "Display: ",
                    choices = c("Count", "Rate", "Fatality Rate"),
                    selected = "Count", inline = TRUE)
     ),
...
server <- function(input, output, session) {
  # reactive dropdown menu
   ...
   all_data <- reactive({covid_US_dat_long %>%
       filter(date <= input$date & Province_State %in% input$state & Admin2 %in% input$county)
   })
   
    track_data <- reactive({all_data() %>% 
            group_by(date) %>% 
   ...
   rank_data <- reactive({all_data() %>%
    ...
    top_county <- reactive({
      (rank_data() %>% filter(date == input$date) %>%
        arrange(desc(get(input$measure)))) [1:min(input$n, nrow(unique(rank_data()[, input$unit]))), input$unit] %>% 
        unlist()
    })
    
    trace_data <- reactive({ 
      rank_data() %>% filter( get(input$unit) %in% top_county() )
    })
    ...
    output$cum_death <- renderPlotly({
      ...
      if (input$death_metric=="Fatality Rate")
      {
    ...
    # first tab summary output
    tot_confirm <- reactive({ 
      format(sum(all_data()$Confirmed), nsmall=1, big.mark=",") 
      })
    tot_death <- reactive({ 
      format(sum(all_data()$Death), nsmall=1, big.mark=",") 
      })
    
    output$summary <- renderText({
      paste("<h1 style=\"color: orange\">", tot_confirm(), "Confirmed Cases</h1>", "<br>", 
            "<h2 style=\"color: red\">", tot_death(), "Deaths</h2>",
            "<br>", "<h4>", "up to", input$date, "</h5>")
    })
